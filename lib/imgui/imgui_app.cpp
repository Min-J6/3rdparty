...
const ImVec4 CLEAR_COLOR        = ImVec4(0.0f, 0.0f, 0.0f, 0.8f);
...
bool ImguiApp::IsRunning() {
    return getInstance().is_running.load();
}

bool ImguiApp::init() {
    // GLFW 초기화
    if (!glfwInit()) {
        printf("[Error]: GLFW 초기화 실패\n");
        return false;
    }
...
bool ImguiApp::run() {
    ...
    while (!glfwWindowShouldClose(window) && is_running.load()) {
        glfwPollEvents();
...
void ImguiApp::startBackground() {
    if (render_thread.joinable()) {
        std::cout << "[Error] [ImGuiApp]" << std::endl;
        return;
    }
