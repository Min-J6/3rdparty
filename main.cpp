        ...
        // 시리얼 모니터 창
        {
            ...
            if (serial.is_open()) {
                ...
                if (ImGui::Button(ICON_MD_LINK " 포트 연결 ") && !available_ports.empty()) {
                    serial.open(available_ports[selected_port_idx], baud_rates[selected_baud_idx]);
                }
            }
            ...
            if (heartbeat_enabled) {
                ...
                if (ImGui::Button(ICON_MD_ECG_HEART " 하트비트 시작") && serial.is_open()) {
            ...
            // 송신 버튼
            if (ImGui::Button(ICON_MD_SEND "보내기") && serial.is_open()) {
                send_data(send_buffer, send_as_hex, send_line_ending);
            }
...
private:
    ...
    void add_received_buffer(const std::string& line_str) {
        if (show_timestamp)
        {
            std::string new_line = "[" + get_timestamp() + "] ";
            new_line += line_str;
            received_lines.push_back(new_line);
        }
        else
        {
    ...
    void start_heartbeat_timer() {
        if (!heartbeat_enabled || !serial.is_open()) return;
        ...
        heartbeat_timer_.async_wait([this](const boost::system::error_code& ec) {
            if (!ec && heartbeat_enabled && serial.is_open()) {
                // 하트비트 데이터 전송
